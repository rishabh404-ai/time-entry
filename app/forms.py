...
class RegisterForm(forms.ModelForm):
    ...
    password2 = forms.CharField(max_length=8, required=True, widget=forms.TextInput(
        attrs={'type': 'password', 'class': "form-control", 'placeholder': "Confirm your Password", 'name': "message", 'required': 'required'}))
...
class LoginForm(forms.Form):
    username = forms.CharField(max_length=100, required=True, widget=forms.TextInput(
        attrs={'type': 'text', 'class': "form-control", 'placeholder': "Enter your Username", 'name': "username"}))
    password = forms.CharField(max_length=50, required=True, widget=forms.TextInput(
        attrs={'type': 'password', 'class': "form-control", 'placeholder': "Create your Password", 'name': "password", 'required': 'required'}))
...
class LogEntryForm(forms.ModelForm):
    ...
    project = forms.ChoiceField(label='Select Your Desired Project', choices=PROJECT_CHOICES, widget=forms.Select(
        attrs={"name": "select_0", "class": "form-control"}))
    ...
    def clean(self):
        ...
        if not start_time:
            raise forms.ValidationError(
                'Failed : Please enter your start time')
